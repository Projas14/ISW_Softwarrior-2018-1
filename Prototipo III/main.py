...
def Monte_Carlo(volatilidad,intereses,Precios,t,iteraciones,Pejercicio):
    ...
    factor = 1
    if iteraciones != 0:
        if iteraciones >= len(Precios):
            Precios_Ut = Precios
        else:
            factor = int(len(Precios)/iteraciones)
            while (Precios[factor*iteraciones]!= Precios[i]):
                Precios_Ut.append(Precios[i])
                i += factor
    ...
    for m in maximos:
       multiplicador += m
    multiplicador = multiplicador/iteraciones
    for r in maximos:
        grafico.append(math.exp(-intereses*t)*r)
    print("esto es grafico ",grafico)
...
def procesar():
    ...
    if Time_maduracion.get() > 3:
        ...
        resultado = Monte_Carlo(volatividad,Tasa_interes.get(),test,1,Iteraciones.get(),Precio_ejecucion.get())
        messagebox.showinfo("ok","El resultado es  de: "+ str(resultado))
        plt.plot(grafico)
        plt.xlabel("numero de iteraciones")
        plt.ylabel("valor opcion")
        plt.show()
...
def procesar2():
	...
	if Time_maduracion.get() > 3:
		...
		resultado = Monte_Carlo(volatividad2,Tasa_interes.get(),DataFinal,1,Iteraciones.get(),Precio_ejecucion.get())
		messagebox.showinfo("ok","El resultado es  de: "+ str(resultado))
		plt.plot(grafico)
		plt.xlabel("numero de iteraciones")
		plt.ylabel("valor opcion")
...
#Iteraciones 
etiqueta_Iteraciones = Label(pes1, text='Iteraciones : ',font=('Governor',10),fg=colorLetra,bg=colorFondo).place(x=420,y=230)
entrada_Iteraciones = Entry(pes1, textvariable=Iteraciones,width=15).place(x=500,y=230)
...
#Iteraciones
etiqueta_Iteraciones = Label(pes2, text='Iteraciones : ',font=('Governor',10),fg=colorLetra,bg=colorFondo).place(x=420,y=230)
entrada_Iteraciones = Entry(pes2, textvariable=Iteraciones2,width=15).place(x=500,y=230)
